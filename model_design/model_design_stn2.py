...
def stn_net_1(inputs):
    ...
    with tf.variable_scope('STN_Net'):
        # --------------------- part1 -------------------------
        pool1 = tf.keras.layers.MaxPool2D(pool_size=(4, 4),
                                          strides=(4, 4),
                                          padding='valid'
                                          )(inputs)
        ...
        bn1 = tf.keras.layers.BatchNormalization(name='stn_bn1')(conv1)
        ...
        print('stn_input_shrink: ', pool2.get_shape().as_list())

        x_flatten = tf.contrib.layers.flatten(inputs=pool2)
...
def fc_net(cnn_out, is_training):
    with tf.variable_scope('FC_Module'):
        ...
        seq_out = tf.reshape(tensor=cnn_out, shape=[-1, shape[1] * shape[2] * shape[3]],
                             name='reshape1')  # (bs, 8, 1024)
